...
var MAIN_MAX_CAP = 200000000000000000000000000;
var DAYS_TO_MAIN_ICO = 80;
var SEND_ETHER =  100;
var SEND_ETHER2 = 50;
...
contract('MainICO', function(accounts) {
  ...
  it("Send 2,000,000,000.000000 DGCT to Crowdsale contract", function() {
    return DGCT.deployed().then(function(coin) {
      ...
      assert.equal(balance.valueOf(),  MAIN_MAX_CAP , "2,000,000,000.000000 wasn't in the Crowdsale account");
    });
  });
 ...
 it("Buy DGCT using 1 ETH should fail, buyer not in whiteList", function() {
    return Crowdsale.deployed().then(function(crowd) {
 ...
 it("change main start time to 2018/4/18 20:0:0", function() {
  ...
  it("Send DGCT to buyers by owner",function(){
    return Crowdsale.deployed().then(function(crowd){
        ...
        return crowd.mainSendDGCT({from:owner}).then(function(){
          return DGCT.deployed().then(function(coin){
            //return coin.balanceOf.call(buyer) ;
            return coin.balanceOf.call(buyer2) ;
          })
            ...
            assert.equal(balance.valueOf(), SEND_ETHER2 * MAIN_DGCT_PER_ETHER, "DGCT not received correct");   
